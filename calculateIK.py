...
class calculateIK():
    ...
    def inverse(self, T0e):
        ...
        exceeds_limits = [False for row in newQ]

        print(newQ)

        for row_idx in range(newQ.shape[0]):
            for theta_idx in range(newQ.shape[1]):
                if (newQ[row_idx, theta_idx] > self.upperLim[0, theta_idx]) or (newQ[row_idx, theta_idx] < self.lowerLim[0, theta_idx]):
